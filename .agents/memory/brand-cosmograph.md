...
- **Canonical logo = v2 "celestial globe" mark:** a lat/long graticule sphere (outer circle + one horizontal + one vertical ellipse) with a 4-point AI star at its core. Stroke `#9277F0`, star fill `#c2b4fb`. Lives in `public/logo-mark.svg` (transparent) + `public/favicon.svg` (same mark inset on a dark `#080a12` rounded badge). The earlier **hexagonal node-network** mark (`attached_assets/galactic-mark-*`) was **v1 — superseded, do not use**. When the user sends a new Claude Design export, the latest dated file wins. **Trap:** the inline `<svg>` inside a Claude/bundler HTML export's `#__bundler_thumbnail` is a stripped-down LOADING PLACEHOLDER (here: only 1 equator + 1 meridian), NOT the real mark. The actual mark is a full graticule sphere (multiple latitudes + meridians). Verify against the user's rendered screenshot / the PDF lead mark, never trust the thumbnail SVG. Built faithfully as concentric ellipses: latitudes `cy=50−40·sinβ, rx=40·cosβ, ry≈13·cosβ` for β=0,±30,±60; meridians `rx=40·sinα, ry=40` for α=30,60 + central line + outer circle.
